SUBROUTINE STEPPC(deltat,errHe,errimp,errvimp)
    ...
    if(ldroplet_frozen)then
        ...
        !$omp parallel do private(ix,iy,iz, tmp_ke, tmp_pot) schedule(static) default(shared) collapse(2)
        ...
        do iz=1,nz; do iy=1,ny
            do ix=1,nx
                ...
                hpsiold(ix,iy,iz,ioldh(2)) = sto4c(ix,iy,iz)
    ...
    if(Ldroplet_frozen)then
        ...
        errHe = 0.d0
        !$omp parallel do private(ix,iy,iz) default(shared) schedule(static) collapse(2) reduction(+:errHe)
        do iz=1,nz; do iy=1,ny
            do ix=1,nx
