...
If(lsolid)Then
    ...
    do iz=1, nz; do iy=1, ny
        do ix=1, nx
            ...
            penalty(ix,iy,iz) = C*(1.d0 + dtemp + beta*den(ix,iy,iz)*(1.d0 - dtemp*dtemp) )
        end do
    end do; end do
    !$omp end parallel do
Endif
...
do iz=1, nz; do iy=1, ny
    do ix=1, nx
        pot4(ix,iy,iz) = delj4(ix,iy,iz) +                                 &   ! Lennard-Jones
                        dencg(ix,iy,iz)*dencg(ix,iy,iz)*(a0+a1*dencg(ix,iy,iz)) +      &   ! Correlation
...
if(core4.eq.'OTC') then
    ...
    do iz=1, nz; do iy=1, ny
        do ix=1, nx
            pot4(ix,iy,iz) = pot4(ix,iy,iz) + ualphas(ix,iy,iz)
        end do
    end do; end do
    !$omp end parallel do
end if
