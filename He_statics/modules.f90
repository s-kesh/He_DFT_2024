...
character  (len=60)           :: filepure       = "pure_density_converged"
end module grid
!------------------------------------------------------------------
module gridk
...
integer (kind=4)                 :: N_imp      ! Number of impurities
real      (kind=8),allocatable   :: rimp(:,:)    ! Impurities position
...
real    (kind=8), allocatable :: Psi(:,:,:)      ! Psi=sqrt(density) in real space
real    (kind=8), allocatable :: Psinw(:,:,:)    ! New value of Psi (temporal)
real    (kind=8), allocatable :: Psi1(:,:,:)     ! Old Psi 1-step  backward
real    (kind=8), allocatable :: Psi2(:,:,:)     ! Old Psi 1-step  backward
...
logical       :: lconstraint_y=.false.
logical       :: lconstraint_x=.false.
logical       :: lconstraint_imp=.false.
real (kind=8) :: Intens = 3000d0 ! Intensity of the constraint.
...
real (kind=8) :: ydist=0d0,xdist=0d0
end module constraint
module rkpc
logical          :: lrkpc=.false.  ! Control to start de Steprkr & Steppcr
logical          :: lrkpcl=.false. ! Control to start de Steprkrl & Steppcrl
integer (kind=4) :: ioldp(3),ioldh(2),Icon_local=0                         ! Auxiliar arrays for steppcr
real    (kind=8), allocatable ::  q(:,:,:), pc(:,:,:), Psil(:,:,:)         ! Auxiliar arrays for Steprkr & Steppcr
real    (kind=8), allocatable ::  Psiold(:,:,:,:), hpsiold(:,:,:,:)        ! Auxiliar arrays for Steppcr
