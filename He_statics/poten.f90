...
real    (kind=8) :: a0,a1
real    (kind=8) :: zcom,ycom,xcom
integer (kind=4) :: ix,iy,iz,iN
real    (kind=8) :: constraint_sum=0d0
!..............................
...
constraint_sum=0d0  
  ...
  ycom = 0.d0
  do iy=1,ny
   ycom = ycom + sum(den(:,iy,:))*y(iy)
  enddo
  ycom = ycom*dxyz/n4real
! print*,'poten.f90-> ycom=',ycom

  If(limp)Then
    yimp = 0.d0
    do iy=1,ny
      yimp = yimp + sum(denx(:,iy,:))*y(iy)
    enddo
    yimp = yimp*dxyz
  Endif
  If(limp.And.Lconstraint_imp)Then
    ...
    forall(ix=1:nx,iy=1:ny,iz=1:nz)
       upotx(ix,iy,iz) = upotx(ix,iy,iz)+Intens*(yimp-ycom-ydist)*y(iy)
    end forall

  Endif

 forall(ix=1:nx,iy=1:ny,iz=1:nz)
       pot4(ix,iy,iz) = pot4(ix,iy,iz)+Intens*(yimp-ycom-ydist)*(-y(iy)/n4real)
    end forall


Endif

If(lconstraint_x)Then

! INTRODUCE CONSTRAINT:

  xcom = 0.d0
  do ix=1,nx
   xcom = xcom + sum(den(ix,:,:))*x(ix)
  enddo
  xcom = xcom*dxyz/n4real
! print*,'ycom=',ycom

  If(limp)Then
    ximp = 0.d0
    do ix=1,nx
      ximp = ximp + sum(denx(ix,:,:))*x(ix)
    enddo
    ximp = ximp*dxyz
  Endif
  If(limp.And.Lconstraint_imp)Then
    ...
    forall(ix=1:nx,iy=1:ny,iz=1:nz)
       upotx(ix,iy,iz) = upotx(ix,iy,iz)+Intens*(ximp-xcom-xdist)*x(ix)
    end forall

  Endif

 forall(ix=1:nx,iy=1:ny,iz=1:nz)
       pot4(ix,iy,iz) = pot4(ix,iy,iz)+Intens*(ximp-xcom-xdist)*(-x(ix)/n4real)
    end forall
