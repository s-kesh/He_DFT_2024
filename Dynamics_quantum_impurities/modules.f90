...
Type, Public  :: info_printout
  ...
  Character (Len=80), Allocatable :: fileimpin_k(:),fileimpout_k(:)
  Character (Len=80), Allocatable :: selec_gs_k(:), selec_gs_k_k(:,:)
  ...
  Real      (Kind=8), Allocatable :: r_cutoff_gs_k(:), r_cutoff_gs_k_k(:,:)
  Real      (Kind=8), Allocatable :: umax_gs_k(:),umax_gs_k_k(:,:)
...
real      (kind=8)               :: mu4          ! Chemical potential for uniform system
...
!------------------------------------------------------------------
module rkpc
integer (kind=4) :: ioldp(3),ioldh(2)      ! Auxiliar arrays for steppc
integer (kind=4), allocatable :: ioldpx(:,:),ioldhx(:,:)       ! Auxiliar arrays for steppc
...
real      (kind=8), allocatable :: uimp_k_k(:,:,:,:,:)  !  Potencial from imp_k for He
...
character (len=80), allocatable :: fileimpin_k(:)
character (len=80), allocatable :: fileimpout_k(:)
