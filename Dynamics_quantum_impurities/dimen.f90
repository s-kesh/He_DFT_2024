...
!   allocate( uext(nx,ny,nz))      ! Fourier transform of the impurity external potential
   Allocate(Ioldpx(3,n_imp)); Allocate(ioldhx(2,N_imp))
   If(Lsolid)allocate(penalty(nx,ny,nz))      ! Potential of penalty term, for the solid functional
   allocate(potx4(nx,ny,nz))       ! X-He potential 
   allocate(uimp_k(nx/2+1,ny,nz,N_imp))   ! Fourier transform of the impurity external potential
   allocate(uimp_k_k(nx/2+1,ny,nz,N_imp,N_imp))   ! Fourier transform of the impurity-impurity  potential
   allocate(psix(nx,ny,nz,N_imp))     ! Array for the impurities w.f.
!   allocate (fdenx(nx/2+1,ny,nz,N_imp))    ! Array with the impurities densities in p-space
   allocate(denx(nx,ny,nz))     ! Density function for the impurities
   allocate(upotx(nx,ny,nz,N_imp))    ! Mean field for the impurities
   allocate(psixold(nx,ny,nz,3,N_imp))     ! Array for old values of impurities w.f.
   allocate(hpsix(nx,ny,nz,N_imp))         ! Array for the hamiltonian of impurities w.f.
   allocate(hpsixold(nx,ny,nz,2,N_imp))    ! Array for old values of hamiltonian of impurities w.f.
   allocate(qx(nx,ny,nz,N_imp)); qx = 0.d0     ! Intermediate array for the impurities w.f.
   allocate(pcx(nx,ny,nz,N_imp)); pcx = 0.d0     ! Intermediate array for the impurities w.f.
   allocate(rimp(N_imp,3))       ! Array test particles
   allocate(filerimp_k(N_imp))
   allocate(fileimpin_k(N_imp))
   allocate(fileimpout_k(N_imp))
