...
logical              :: lbulk=.false.   ! T-> For uniform o seminfinite systems
...
real       (kind=8)  :: errHe, errimp    ! Error evolution (only form Predictor-Corrector-Modificator)
real       (kind=8)  :: Zsurf = -25.d0, Morse_HeTiO2_1D, Morse_HeTiO2_3D, auxn4 ! Position of the surface
real       (kind=8)  :: auxx,auxy,auxz,sto
...
namelist /input/title,fftwplan,nthread,nsfiles,                         &
                ...
                txmean,txsurf,tymean,tysurf,tzmean,tzsurf,Lbulk,        &
                Lparabolic_fit_FT_V_spline

namelist /imp/m_imp_u,selec_gs_k,r_cutoff_gs_k,umax_gs_k,               &
              selec_gs_k_k,r_cutoff_gs_k_k,umax_gs_k_k,                 &         
              filerimp_k,fileimpin_k
...
If(lbulk)then
 Call rhoasin0(aux,densat4,mu4,aux1)
 Write(6,'(" Potencial quimic a la saturacio......:",1p,E18.9)')mu4
Else
 mu4=0.d0 
Endif
...
Do ix=1,N_imp
  Fileimpin_K(ix) = Trim(Adjustl(Fileimpin_K(ix)))
  Filerimp_K(ix) = Trim(Adjustl(Filerimp_K(ix)))
EndDo
...
select case(mode)
   ...
   case(2) !................................... Continue a dynamic calculation from a prevous one
      write(6,6013) filedenout,(fileimpout_k(ix),ix=1,N_imp)
...
!
!  Escriu el potencial que veu l'heli al llarg del eix Z
!

Open(Unit=1,File="potx4-z.dat")
Do iz=1, nz
  Write(1,'(1p,2E15.6)')z(iz),potx4(nx/2+1,ny/2+1,iz)
EndDo  
Close(1)

Open(Unit=1,File="pot4-z.dat")
Do iz=1, nz
  Write(1,'(1p,2E15.6)')z(iz),pot4(nx/2+1,ny/2+1,iz)
EndDo  
Close(1)
...
Do ix=1, N_imp
  Aux = Sum(Conjg(Psix(:,:,:,ix))*Psix(:,:,:,ix))*dxyz
!      aux =sum(denx)*dxyz
  Write(6,'(" W.f.(",I2,")normalization.....:",1p,E18.10)')ix,aux
EndDo
...
do k=1,N_imp
  If(iter0.Eq.0)write((100+k),'("# Tiempo(ps), x(AA), y(AA), z(AA)")')
  auxx = 0.d0; auxy = 0.d0; auxz = 0.d0; aux = 0.d0
  Do ix = 1, nx
    Do iy = 1, ny
      Do iz = 1, nz
        Sto = Conjg(Psix(ix,iy,iz,k))*Psix(ix,iy,iz,k)
        aux = aux + Sto
        auxx = auxx + Sto*x(ix)
        auxy = auxy + Sto*y(iy)
        auxz = auxz + Sto*z(iz)
  ...
  Auxy = Auxy*dxyz/Aux
  Auxz = Auxz*dxyz/Aux
  write((100+k),'(1x,1p,E15.6,3E18.10)')time0, Auxx, Auxy, Auxz
enddo
...
pr%it = iter
    ...
    if((iter-iter0+1).le.3.Or.lrk)then
      ...
      write(6,'(" Error( He, imp) (From Steppc)...",1p,3E15.6)')errHe,errimp
    endif
   ...
   do k=1,N_imp
     ...
     Auxy = Auxy*dxyz/Aux
     Auxz = Auxz*dxyz/Aux
     write((100+k),'(1x,1p,E15.6,3E18.10)')aux1, Auxx, Auxy, Auxz
   enddo
...
!      write(chariter,8013)ncurr
      ...
      namefile='density.'//Trim(chariter)//'.dat'
      pr%namefile = namefile
      pr%psi(:,:,:) = psi(:,:,:)
       call printoutc(pr)
       Do ix=1, N_imp
         Write(numero,'(I2)')ix
         namefile='denX_'//Trim(Adjustl(numero))//'.'//Trim(chariter)//'.dat'
         pr%namefile = namefile
         pr%psi(:,:,:) = psix(:,:,:,ix)
         call printoutc(pr)
       EndDo
